# python -m PyInstaller --onefile WORK_FILE_21_07.py
import logging
import time
...
NEW_BOARD_CELL_LIMIT = 60 # Максимальное кол-во плат при котором если робот его превышпет он уходит в режим досушки
...
class OPCClient:
    ...
    def update_registers(self):
        ...
        global shared_data, Cell3, Order
...
class Table:
    ...
    def _send_robot_command(self, base_command, cell_num=None, timeout_s: int = 7200):
        """
        ...
        elif base_command == 250:  # Перевод робота в нулевую точку
            command = 250  # 250
        else:
        ...
        try:
            while time.time() - start_time < timeout_s:
                if EMERGENCY_STOP.is_set():
                    ...
                    logger4.error(f"СТОЛ {self.number} Остановка по аварийному тайм ауту = {timeout_s}, время ожидания {start_time}")
                    return False
    ...
    def _place_new_board_with_photo(
        ...
        for attempt in range(1, max_new_board_tries + 1):
            ...
            # Отдельная проверка на достижение 60 при заборе из тары новых плат. когда кончились переводим в досушку
            # Если уже брали плату из ячейки 60 — новые платы больше не берём
            # Все 60 ячеек тары с новыми платами уже использованы
            with shared_data_lock:
                cell1_limit_reached = Cell1 >= NEW_BOARD_CELL_LIMIT

            if cell1_limit_reached:
                logger4.warning(
                    f"[MAIN] СТОЛ {self.number}: обработана последняя ячейка "
                    f"тары новых плат Cell1={Cell1}. Переходим в режим досушки"
                )

                NO_MORE_NEW_BOARDS.set()
                return None

            # 1. Готовим номер ячейки, из которой робот возьмёт новую плату
            Tray_robot = 1
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Дожидаемся завершения потоков столов
        for t in (thread1, thread2, thread3):
            try:
                if t.is_alive():
                    t.join(timeout=30)
            except Exception:
                logger4.exception("[MAIN] Ошибка ожидания потока стола")

        # Проверяем, что все столы действительно завершились
        all_tables_stopped = all(
            not t.is_alive()
            for t in (thread1, thread2, thread3)
        )

        # После штатной досушки отправляем робота в нулевую позицию
        if (
            NO_MORE_NEW_BOARDS.is_set()
            and not EMERGENCY_STOP.is_set()
            and all_tables_stopped
        ):
            logger4.warning("[MAIN] Досушка завершена. Отправляем роботу команду 250")
            print(f"[MAIN] Досушка завершена. Отправляем роботу команду 250")

            if table1._send_robot_command(250):
                logger4.info("[MAIN] Робот успешно переведён в нулевую позицию")
                print(f"[MAIN] Робот успешно переведён в нулевую позицию")
            else:
                logger4.error("[MAIN] Робот не подтвердил выполнение команды 250")
                print(f"[MAIN] Робот не подтвердил выполнение команды 250")

        # После команды 250 останавливаем OPC
        try:
            opc_client.stop()
        except Exception:
            logger4.exception("[MAIN] Ошибка при остановке OPC клиента")

        # Останавливаем синхронизацию БД
        try:
            db_sync.stop()
        except Exception:
